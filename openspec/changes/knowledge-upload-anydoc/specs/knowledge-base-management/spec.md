...
## Purpose
Update the knowledge base management behavior: knowledge content is now built from explicitly uploaded files converted to Markdown, uploaded via a dedicated entry on the Knowledge page.

## MODIFIED Requirements

### Requirement: System provides knowledge base management
The system SHALL provide a knowledge base page listing all knowledge bases with file counts, SHALL provide a dedicated file upload entry on the Knowledge page, and SHALL support uploading files into a knowledge base where non-Markdown files are converted to Markdown before being stored as knowledge content.
...
#### Scenario: Upload file from the Knowledge page
- **WHEN** user selects a file on the Knowledge page and uploads it to a knowledge base
- **THEN** the system SHALL check for duplicates, convert the file to Markdown, store it, and make its content searchable
- **AND** duplicate files SHALL be rejected with a message

#### Scenario: Knowledge content comes from uploaded files
- **WHEN** the knowledge base is queried by RAG search
- **THEN** the system SHALL retrieve content from Markdown documents produced by uploaded files
- **AND** chat conversation content SHALL NOT be added to the knowledge base automatically
