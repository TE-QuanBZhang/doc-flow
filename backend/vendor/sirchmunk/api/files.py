...
from ..utils.anydoc_converter import convert_to_markdown
...
def _knowledge_root() -> Path:
    """Directory holding ingested Markdown, one subdir per collection."""
    work_path = Path(
        os.getenv("SIRCHMUNK_WORK_PATH", os.path.expanduser("~/.sirchmunk"))
    ).expanduser().resolve()
    return work_path / "knowledge"
...
async def upload_files(
    ...
    for i, upload_file in enumerate(files):
        try:
            ...
            # Convert to Markdown and ingest into the knowledge directory
            filename = upload_file.filename or "unnamed"
            try:
                markdown = convert_to_markdown(content, filename)
                kb_dir = _knowledge_root() / collection
                stem, _ = os.path.splitext(relative_path or filename)
                kb_path = kb_dir / f"{stem}.md"
                kb_path.parent.mkdir(parents=True, exist_ok=True)
                kb_path.write_text(markdown, encoding="utf-8")
                conversions.append({
                    "name": filename,
                    "status": "converted" if os.path.splitext(filename)[1].lower() not in (".md", ".markdown") else "passed_through",
                    "path": str(kb_path),
                })
            except Exception as e:
                logger.warning("Failed to convert %s: %s", filename, e)
                conversions.append({"name": filename, "status": "failed", "error": str(e)})
        except ValueError as e:
...
async def delete_collection(name: str):
    ...
    try:
        ...
        # Clean up the ingested Markdown for this collection
        kb_dir = _knowledge_root() / name
        if kb_dir.exists():
