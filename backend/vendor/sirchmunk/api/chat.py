...
def _resolve_rag_paths(kb_name: str) -> Tuple[List[str], str]:
    ...
    if not paths:
        # Default: the knowledge documents directory (ingested Markdown)
        work_path = os.path.expanduser(os.getenv("SIRCHMUNK_WORK_PATH", "~/.sirchmunk"))
        kb_dir = os.path.join(work_path, "knowledge")
