...
async def list_knowledge_documents():
    """List ingested Markdown documents in the knowledge directory, grouped by collection."""
    work_path = Path(
        os.getenv("SIRCHMUNK_WORK_PATH", os.path.expanduser("~/.sirchmunk"))
    ).expanduser().resolve()
    kb_root = work_path / "knowledge"
    if not kb_root.is_dir():
        return {"success": True, "data": {"collections": [], "total": 0}}

    collections = []
    total = 0
    for col_dir in sorted(kb_root.iterdir()):
        if not col_dir.is_dir():
            continue
        docs = []
        for f in sorted(col_dir.rglob("*.md")):
            st = f.stat()
            docs.append({
                "name": str(f.relative_to(col_dir)),
                "size_bytes": st.st_size,
                "modified": datetime.fromtimestamp(st.st_mtime).isoformat(),
            })
        if docs:
            collections.append({"collection": col_dir.name, "documents": docs, "count": len(docs)})
            total += len(docs)
    return {"success": True, "data": {"collections": collections, "total": total}}
