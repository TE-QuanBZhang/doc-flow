...
class EmbeddingUtil:
    ...
    def _download_model(model_id: str, cache_dir: Optional[str] = None) -> str:
        ...
        ignore = ["openvino/*", "onnx/*", "model.onnx", "pytorch_model.bin", "rust_model.ot", "tf_model.h5"]
        ...
        try:
            ...
            # Cache metadata may survive an interrupted download while the
            # weight file is missing. Verify a real weight file before
            # trusting the cache, otherwise fall through to online download.
            weight_candidates = ("model.safetensors", "pytorch_model.bin", "tf_model.h5")
            if not any(
                os.path.exists(os.path.join(model_dir, w)) for w in weight_candidates
            ):
                raise FileNotFoundError(f"No weight file in cache: {model_dir}")
