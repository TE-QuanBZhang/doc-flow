...
export default function KnowledgePage() {
  ...
  const [docCollections, setDocCollections] = useState<KnowledgeCollection[]>([]);
  ...
  const fetchDocuments = async () => {
    try {
      const response = await fetch(apiUrl("/api/v1/knowledge/documents"), { headers: { ...getAuthHeaders() } });
      if (!response.ok) return;

      const result = await response.json();
      if (result.success && result.data) {
        setDocCollections(result.data.collections || []);
        setDocTotal(result.data.total || 0);
      }
    } catch (err: any) {
  ...
  const handleRefresh = async () => {
    ...
    await Promise.all([fetchStats(false), fetchPatterns(), fetchDocuments()]);
  };
        ...
        <div className="bg-white dark:bg-slate-800 rounded-xl shadow-sm border border-slate-200 dark:border-slate-700 p-6">
          <div className="flex items-center gap-2 mb-4">
            ...
            <h3 className="text-lg font-semibold text-slate-900 dark:text-slate-100">
              {t("Knowledge Documents")} ({docTotal})
            </h3>
          </div>
          {docCollections.length === 0 ? (
            <p className="text-sm text-slate-500 dark:text-slate-400">
              {t("No documents yet. Upload files to build the knowledge base.")}
            </p>
          ) : (
            <div className="space-y-4">
              {docCollections.map((col) => (
                <div key={col.collection}>
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-2">
                    {col.documents.map((doc) => (
                      <div
                        key={doc.name}
                        className="flex items-center justify-between px-3 py-2 bg-slate-50 dark:bg-slate-700/40 rounded-lg"
                      >
                        <span className="text-sm text-slate-700 dark:text-slate-300 truncate mr-2" title={doc.name}>
                          {doc.name}
                        </span>
                        <span className="text-xs text-slate-500 dark:text-slate-400 whitespace-nowrap">
                          {doc.size_bytes < 1024 ? `${doc.size_bytes} B` : `${(doc.size_bytes / 1024).toFixed(1)} KB`}
                        </span>
                      </div>
                    ))}
                  </div>
